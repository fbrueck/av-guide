...
def _match_context(cfg):
    ...
    return entries, pois, index, keys, decisions, verdicts, cfg
...
def _rebuild_funnel_via_classify(cfg) -> dict[str, dict[str, int]]:
    ...
    _e, _p, index, keys, decisions, verdicts, _c = _match_context(cfg)
    ...
    for entry in sorted(load_jsonl(cfg.routes_jsonl), key=lambda e: e["id"]):
        ...
        for item in items:
            method = classify_method(
                item, entry["id"], index, keys, decisions, verdicts, cfg
            )
...
def test_method_is_per_link_not_pois_best_method(cfg):
    ...
    poi = next(p for p in pois.values() if p["osm"] == "node/8001")
