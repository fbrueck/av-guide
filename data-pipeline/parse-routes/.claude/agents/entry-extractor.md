...
- `start_quote` — a **short verbatim snippet** (≈ the first 6–12 words) copied
  **exactly as printed** from where the entry's text begins: its heading line,
  ...
  be **unique on the page**. Route/Traverse headings often repeat verbatim on the
  same page (`Von Süden`, `Südwestgrat`, `Übergang zur …`, `Abstieg`); when the
  heading alone is not unique, **continue past it into the first words of the
  body** (the metadata block or opening prose) until the snippet is unique — a
  too-short repeated start is the single most common slicing failure.
- `end_quote` — a **short verbatim snippet** (≈ the last 6–12 words, including the
  ...
  **ends**, i.e. its final words just before the next entry's Randziffer. If the
  ...
  including this snippet, so it must be the entry's **true tail** — long enough to
  be unique, and **not** a phrase that also appears earlier inside the same entry.
  Three tail mistakes each break the slice, so avoid them:
  - **Never echo the heading.** The end anchor is the entry's *last* words, not a
    repeat of its opening line/grade. For a multi-paragraph entry the tail is the
    final sentence of the last paragraph, not something near the top.
  - **It must follow the start anchor and not overlap it.** For a one-sentence
    entry, `end_quote` is the words *after* the `start_quote` snippet (e.g. start
    `Aussichtspunkt unmittelbar nordöstl.`, end `nordöstl. über Hinterriß.` would
    overlap — use `über Hinterriß.` instead). Start and end must not share words.
  - **Copy char-for-char, don't tidy.** Reproduce the page's OCR quirks in the
    tail exactly (umlauts, `ß`, `>-451` arrow cross-refs, `®`); a "cleaned-up"
    tail is not on the page and will not be found.
