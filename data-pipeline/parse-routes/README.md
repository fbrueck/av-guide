...
`routes.json` is the **route-map** webapp's contract: the same routes as a plain
JSON array, projected to a stable field set (see below) so the browser loads
Route metadata without parsing JSONL. It is written by `pipeline.export` and
...
`route_id`, `name`, `peak`, `grade`, `time`, `height_m`, `first_ascent`,
`summary`, `description` — dropping internal bookkeeping like `source_page`.
That projection is what `route-map` depends on; keep it in step with the
webapp's data contract (`route-map/CLAUDE.md`) when either changes.
