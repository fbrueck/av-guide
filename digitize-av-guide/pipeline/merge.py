...
(`data/03_structured/parts/page_0051.json`) containing the routes that *start*
on that page. This step explodes those into the final artifact: one JSON file
per route under `data/03_structured/routes/`, each self-contained with the
...
def main() -> None:
    if not config.STRUCT_PARTS.exists():
    ...
    if config.ROUTES_DIR.exists():
        shutil.rmtree(config.ROUTES_DIR)
    config.ROUTES_DIR.mkdir(parents=True, exist_ok=True)

    all_routes: list[dict] = []
    bad = 0
    for part in sorted(config.STRUCT_PARTS.glob("page_*.json")):
        ...
        for seq, route in enumerate(data.get("routes", []), start=1):
            ...
            (config.ROUTES_DIR / f"{route_id}.json").write_text(
                json.dumps(record, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
            )
            all_routes.append(record)

    all_routes.sort(key=lambda r: r["route_id"])
    with config.ROUTES_JSONL.open("w", encoding="utf-8") as f:
        for route in all_routes:
            f.write(json.dumps(route, ensure_ascii=False) + "\n")

    n_pages = len(list(config.STRUCT_PARTS.glob("page_*.json")))
    print(f"Wrote {len(all_routes)} route files -> {config.ROUTES_DIR}")
    print(f"Combined index -> {config.ROUTES_JSONL}  ({n_pages} pages, {bad} unreadable)")
