...
export function joinGuideData(raw: RawArtifacts): GuideData {
	...
	function resolveTargetPlace(
		...
		// The Place's routes-leading-here list = only the routes whose Ziel
		// (Destination) is this Place, i.e. routes that END here — not waypoints
		// passing through. A route has one Destination, so at most one push per
		// (route, place); the includes() guard stays as a cheap defence.
